...
// ══════════════════════════════════════════════════════════
// 2. واجهات الأنماط الصارمة (Interfaces)
// ══════════════════════════════════════════════════════════

export interface Department { name: string; order: number; }
export interface Job { name: string; }
export interface ServiceType { name: string; order: number; domain: DomainType; defaultDept: string; }
export interface TransactionType { name: string; order: number; contractPath: ContractPath; targetDepartment?: string; allowedServiceTypes: string[]; }
export interface Governorate { name: string; order: number; }
export interface Area { name: string; }
...
export const defaultTransactionTypes: TransactionType[] = [
    { name: 'عقد خدمات فقط (استشاري وتصميم)', order: 0, contractPath: ContractPath.SERVICES_ONLY, targetDepartment: 'الهندسة والتصميم الفكري', allowedServiceTypes: ['خدمات استشارية'] },
    { name: 'عقد تنفيذ فقط (مقاولات وبناء)', order: 1, contractPath: ContractPath.EXECUTION_ONLY, targetDepartment: 'إدارة المشاريع والتنفيذ', allowedServiceTypes: ['خدمات تنفيذية'] },
    { name: 'عقد هجين (خدمات مع تنفيذ)', order: 2, contractPath: ContractPath.HYBRID_PATH, allowedServiceTypes: ['خدمات استشارية', 'خدمات تنفيذية'] }
];

export const defaultServiceTypes: ServiceType[] = [
    { name: 'خدمات استشارية', order: 0, domain: DomainType.INTELLECTUAL, defaultDept: 'الهندسة والتصميم الفكري' },
    { name: 'خدمات تنفيذية', order: 1, domain: DomainType.FIELD_MATERIAL, defaultDept: 'إدارة المشاريع والتنفيذ' },
    { name: 'خدمات صيانة', order: 2, domain: DomainType.MAINTENANCE, defaultDept: 'إدارة المشاريع والتنفيذ' },
];
...
export const defaultItemCategories = [
    { code: 'GEN', name: 'عام', description: 'فئة عامة للأصناف غير المصنفة' },
    { code: 'CONS', name: 'مواد استهلاكية', description: 'المواد التي تستهلك أثناء التشغيل' },
    { code: 'EQUIP', name: 'معدات', description: 'المعدات والأدوات المستخدمة' },
    { code: 'SPARE', name: 'قطع غيار', description: 'قطع الغيار للمعدات والآلات' },
];
