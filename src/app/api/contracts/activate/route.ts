...
export async function POST(request: NextRequest) {
  try {
    ...
    const { transactionId, clientId, clauses, totalAmount, financialsType, quotationIdToUpdate } = body;
    if (!clientId || !clauses || clauses.length === 0) return NextResponse.json({ success: false, error: 'بيانات ناقصة' }, { status: 400 });
    let contract;
    if (transactionId) {
      ...
      const tx = await db.clientTransaction.create({ data: { transactionNumber: `TRX-${Date.now()}`, clientId, transactionType: 'contract', status: 'in-progress', totalAmount } });
      contract = await db.contract.create({ data: { transactionId: tx.id, totalAmount, status: 'active', startDate: new Date() } });
    }
    ...
    if (quotationIdToUpdate) await db.quotation.update({ where: { id: quotationIdToUpdate }, data: { status: 'accepted' } });
