...
import { ArrowRight, Banknote, FileText, Target, Printer, Pencil } from 'lucide-react';
...
export default function ViewCashReceiptPage() {
  ...
  // 🛡️ توجيه مسار الاستماع للمنظومة المعزولة 🛡️
  const receiptPath = useMemo(() => id && tenantId ? getTenantPath(`cashReceipts/${id}`, tenantId) : null, [id, tenantId]);
  const { data: receipt, loading } = useDocument<CashReceipt>(firestore, receiptPath);

  if (loading) return <div className="max-w-4xl mx-auto p-8"><Skeleton className="h-96 w-full rounded-[2.5rem]" /></div>;
  if (!receipt) return <div className="text-center py-20 font-black opacity-30">لم يتم العثور على السند في مسار منشأتك.</div>;
        ...
        <div className="space-y-10">
            ...
            <main className="space-y-8 bg-slate-50/50 p-10 rounded-[2.5rem] border-2 border-slate-100">
                ...
                <div className="grid grid-cols-2 gap-10">
                    <InfoRow label="تاريخ السند" value={apptDate ? format(apptDate, 'eeee, dd MMMM yyyy', { locale: ar }) : '-'} />
                    <InfoRow label="طريقة الدفع" value={<Badge className="bg-white border-2 text-primary font-black px-4 h-7 rounded-full">{receipt.paymentMethod}</Badge>} />
                </div>
                ...
                {receipt.projectNameAr && (
                    <div className="flex items-center gap-4 p-6 bg-primary/5 rounded-3xl border border-primary/10">
                        <div className="p-3 bg-white rounded-2xl shadow-sm text-primary"><Target className="h-6 w-6" /></div>
                        <div>
