...
type Employee = { hireDate?: Date | string | null; basicSalary?: number; housingAllowance?: number; transportAllowance?: number; annualLeaveUsed?: number; carriedLeaveDays?: number; terminationReason?: string | null; };
type Holiday = { name: string; date: Date | string; };
import { toDateSafe } from './date-converter';
...
export const calculateAnnualLeaveBalance = (employee: Partial<Employee>, asOfDate: Date): number => {
    const hireDate = toDateSafe(employee.hireDate);
