...
export function SupplierQuotationCard({ rfq, vendor, existingQuote }: SupplierQuotationCardProps) {
  ...
  const formattedDate = (() => {
    ...
    try {
      const parsed = toFirestoreDate(existingQuote.date);
    ...
    <>
      <Card
        className={cn(
          ...
          existingQuote
            ? 'border-green-500/20 bg-green-50/10 shadow-lg'
            : 'border-muted hover:border-primary/30'
        )}
      >
        <CardHeader className={cn('pb-4 border-b', existingQuote ? 'bg-green-500/5' : 'bg-muted/30')}>
          <div className="flex justify-between items-start">
            ...
            {existingQuote && (
              <div className="bg-green-100 text-green-700 px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1">
                <CheckCircle className="h-3 w-3" /> عرض مستلم
              </div>
            )}
          </div>
