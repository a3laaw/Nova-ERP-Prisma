...
export function RfqComparisonView({ rfq }: RfqComparisonViewProps) {
  ...
  const handleConfirmSplitAward = async () => {
    ...
    try {
        ...
        toast({ title: 'نجاح الترسية السيادية', description: 'تم تحويل اختياراتك إلى أوامر شراء رسمية بنجاح.' });
            ...
            {tableData.map(({ item, quotes, minPrice }) => (
              <TableRow key={item.id} className="h-16 hover:bg-muted/5 transition-colors border-b last:border-0 group">
                ...
                {allVendors.map(vendor => {
                  ...
                  return (
                    <TableCell key={vendor.id} className={cn("text-center border-r p-0 min-w-[140px] transition-all", !isLocked && "cursor-pointer", isSelected && "bg-primary/5 border-2 border-primary shadow-inner")} onClick={() => handleCellClick(item.id, vendor.id!, quote?.price || 0)}>
                      {quote?.price ? (
          ...
          {!isLocked ? (
              <div className="flex justify-between items-center p-8 bg-primary/5 rounded-[2.5rem] border-2 border-primary/10 shadow-2xl animate-in slide-in-from-bottom-4">
                  <div className="flex items-center gap-6">
                      ...
                      <div>
                          <h4 className="font-black text-2xl text-primary tracking-tight">اعتماد قرار الشراء والترسية</h4>
              ...
              <div className="p-8 bg-green-50 border-2 border-dashed border-green-200 rounded-[2.5rem] flex flex-col md:flex-row items-center justify-between gap-6 shadow-sm">
                  <div className="flex items-center gap-4">
                      ...
                      <div>
                          <p className="font-black text-green-800 text-2xl tracking-tight">تمت الترسية وإصدار أوامر الشراء</p>
                          <p className="text-sm text-green-700 font-bold">المصفوفة مؤرشفة حالياً ومرتبطة بـ {rfq.awardedPoIds?.length} أوامر شراء نشطة.</p>
                      </div>
                  </div>
              </div>
          )}
