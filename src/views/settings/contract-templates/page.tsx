...
import { PlusCircle, Pencil, Trash2, FileSignature, Briefcase, Construction } from 'lucide-react';
...
export default function ContractTemplatesManagementPage() {
      ...
      >
        ...
        <TabsContent value="Consulting" className="animate-in fade-in slide-in-from-right-4 duration-300">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {loading ? <Skeleton className="h-32 w-full" /> : consultingTemplates.map(t => <TemplateCard key={t.id} template={t} onEdit={handleEdit} onDelete={setTemplateToDelete} />)}
          </div>
        </TabsContent>

        <TabsContent value="Execution" className="animate-in fade-in slide-in-from-left-4 duration-300">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {loading ? <Skeleton className="h-32 w-full" /> : executionTemplates.map(t => <TemplateCard key={t.id} template={t} onEdit={handleEdit} onDelete={setTemplateToDelete} />)}
          </div>
        </TabsContent>
      </Tabs>
...
function TemplateCard({ template, onEdit, onDelete }: { template: ContractTemplate, onEdit: any, onDelete: any }) {
    return (
        <Card className="group hover:shadow-md transition-all border-2 border-transparent rounded-2xl overflow-hidden hover:border-primary/20">
            <CardHeader className="pb-3 bg-muted/10">
                ...
                <CardDescription className="line-clamp-2 text-xs min-h-[32px]">{template.description || 'لا يوجد وصف متاح.'}</CardDescription>
            </CardHeader>
            <CardContent className="pt-4 flex justify-between items-center text-[10px] font-bold text-muted-foreground">
