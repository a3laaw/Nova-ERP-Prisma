...
export function UsersTable() {
    ...
    const quotaInfo = useMemo(() => {
        ...
        return { totalLimit, usedCount, isFull: usedCount >= totalLimit };
    }, [currentCompany, users]);

    /**
     * محرك الحفظ السيادي: يتم عبر الـ API الخلفي لضمان تجاوز قيود المتصفح الأمنية.
     */
    const handleSaveUser = async (userData: Partial<UserProfile> & { newPassword?: string }) => {
        ...
        try {
            ...
            if (result.success) {
                toast({ title: '✅ تم تفعيل الحساب', description: 'الموظف جاهز الآن لتسجيل الدخول باسم المستخدم.' });
                setIsFormOpen(false);
                fetchUsersAndEmployees();
            } else {
                throw new Error(result.error || 'فشل محرك التفعيل.');
            }
        } catch (error: any) {
    ...
    const handleToggleActivation = async (user: UserWithEmployee) => {
        ...
        try {
            await updateDoc(doc(firestore, usersPath), { isActive: !user.isActive, updatedAt: serverTimestamp() });
    ...
    const handleDeleteUser = async (user: UserWithEmployee) => {
        ...
        try {
            ...
            batch.delete(doc(firestore, getTenantPath(`users/${user.id}`, tenantId)));
            ...
            fetchUsersAndEmployees();
        } catch (e) { toast({ variant: 'destructive', title: 'خطأ في الحذف' }); }
    };
    ...
    <div className="space-y-8" dir="rtl">
        <div className="flex flex-col lg:flex-row justify-between items-center gap-6">
            <div className="flex items-center gap-4">
                <div className="p-3 bg-primary/10 rounded-2xl text-primary shadow-inner"><ShieldCheck className="h-8 w-8" /></div>
                <div>
                    <h2 className="text-2xl font-black text-[#1e1b4b]">إدارة حسابات الدخول</h2>
                    <p className="text-sm font-bold text-slate-500 mt-1">تفعيل صلاحيات دخول المهندسين والموظفين باستخدام أسماء المستخدمين.</p>
                </div>
            </div>
            ...
            <div className="flex items-center gap-4 bg-white/50 p-2 rounded-2xl border shadow-inner">
                <div className="px-6 py-1 border-l border-slate-200 text-center">
                    ...
                    <div className="flex items-center justify-center gap-2">
                        <span className={cn("text-2xl font-black font-mono", quotaInfo.isFull ? "text-red-600" : "text-primary")}>{quotaInfo.usedCount}</span>
                        <span className="text-slate-400 font-black text-xl">/</span>
                        <span className="text-slate-500 font-black text-2xl font-mono">{quotaInfo.totalLimit}</span>
                    </div>
                </div>
                <Button onClick={() => { setSelectedUser(null); setIsFormOpen(true); }} disabled={quotaInfo.isFull || loading} className="h-12 rounded-xl font-black gap-2 shadow-lg"><PlusCircle className="h-5 w-5" /> إضافة حساب</Button>
            </div>
        </div>
        ...
        <Card className="rounded-[3rem] border-none shadow-xl overflow-hidden bg-white">
            <CardHeader className="bg-muted/10 border-b p-8 px-10">
                <div className="relative w-full max-w-md">
                    ...
                    <Input placeholder="بحث باسم المستخدم أو الموظف..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 h-12 rounded-2xl bg-white border-none shadow-inner font-black text-[#1e1b4b]" />
                </div>
            </CardHeader>
                ...
                <Table>
                    <TableHeader className="bg-[#F8F9FE] h-14">
                        <TableRow className="border-none">
                            <TableHead className="px-10 font-black text-[#7209B7]">اسم المستخدم (Username)</TableHead>
                    ...
                    <TableBody>
                        {loading ? (
                            ...
                            <TableRow><TableCell colSpan={5} className="h-48 text-center text-slate-300 font-black italic">لا توجد حسابات مسجلة حالياً.</TableCell></TableRow>
                        ) : (
                            filteredUsers.map((user) => (
                                <TableRow key={user.id} className="hover:bg-primary/[0.02] transition-all h-24 border-b last:border-0 group">
                                    <TableCell className="px-10">
                                        <div className="flex flex-col">
                                            <span className="font-mono text-lg font-black text-[#1e1b4b]">@{user.username}</span>
                                            <span className="text-[9px] text-slate-400 font-mono opacity-60">ID: {user.id.substring(0,8)}...</span>
                                        </div>
                                    </TableCell>
                                    <TableCell className="font-black text-slate-800">{user.employeeFullName || 'غير مرتبط بملف'}</TableCell>
                                    <TableCell><Badge variant="outline" className={cn("px-4 py-1 rounded-xl font-black text-[10px] border-2", roleColors[user.role])}>{roleTranslations[user.role]}</Badge></TableCell>
                                    <TableCell className="text-center"><Badge variant={user.isActive ? 'default' : 'secondary'} className={cn("px-4 py-1 rounded-xl font-black text-[10px]", user.isActive ? 'bg-green-600 text-white' : 'bg-slate-100 text-slate-500')}>{user.isActive ? 'نشط' : 'موقف'}</Badge></TableCell>
                                    <TableCell className="text-center">
                                       {currentUser?.id !== user.id && (
                                        <DropdownMenu>
                                            <DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-11 w-11 rounded-2xl border bg-white shadow-sm opacity-0 group-hover:opacity-100 transition-opacity"><MoreHorizontal className="h-5 w-5" /></Button></DropdownMenuTrigger>
                                            <DropdownMenuContent align="end" dir="rtl" className="rounded-2xl shadow-2xl border-none p-2 bg-white/95 backdrop-blur-xl">
                                                ...
                                                <DropdownMenuItem onClick={() => { setSelectedUser(user); setIsFormOpen(true); }} className="gap-2 rounded-xl py-3 font-bold text-[#1e1b4b]"><Pencil className="h-4 w-4 text-primary" /> تعديل البيانات</DropdownMenuItem>
                                                <DropdownMenuItem onClick={() => handleToggleActivation(user)} className={cn("gap-2 rounded-xl py-3 font-bold", user.isActive ? "text-orange-600" : "text-green-600")}>{user.isActive ? <UserX className="h-4 w-4"/> : <UserCheck className="h-4 w-4"/>} {user.isActive ? 'إيقاف الدخول' : 'تفعيل الدخول'}</DropdownMenuItem>
                                                <DropdownMenuSeparator />
                                                <DropdownMenuItem className="text-red-600 gap-2 rounded-xl py-3 font-bold focus:bg-red-50" onClick={() => handleDeleteUser(user)}><Trash2 className="h-4 w-4" /> حذف نهائياً</DropdownMenuItem>
                                            </DropdownMenuContent>
                                        </DropdownMenu>
                                       )}
                                    </TableCell>
        ...
        {isFormOpen && (
            <UserForm isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} onSave={handleSaveUser} user={selectedUser} employees={employees} allUsers={users} isSaving={isSaving} />
        )}
    </div>
    );
}
